...
/--
  A principal submodule is generated by one element, called `submodule.is_principal.generator`.
 -/
class submodule.is_principal [comm_ring α] [add_comm_group β] [module α β] (S : submodule α β) : Prop :=
(principal : ∃ (x : β), S = submodule.span _ {x})

section prio
set_option default_priority 100 -- see Note [default priority]

/-- A principal ideal domain is an integral domain where each ideal is principal. -/
...
variables [comm_ring α] [add_comm_group β] [module α β]
open submodule

/--
  The generator of a principal submodule `S` is the `x ∈ S` such that `S = span _ {x}`.

  A submodule might have many generators, for example differing by multiplication by a unit.
  This is reflected in the `noncomputable` aspect of this definition.
-/
noncomputable def generator (S : submodule α β) [S.is_principal] : β :=
classical.some (principal S)

lemma span_singleton_generator (S : submodule α β) [S.is_principal] : span _ {generator S} = S :=
eq.symm (classical.some_spec (principal S))

@[simp] lemma generator_mem (S : submodule α β) [S.is_principal] : generator S ∈ S :=
by conv {to_rhs, rw ← span_singleton_generator S}; exact submodule.subset_span (mem_singleton _)

lemma mem_iff_eq_smul_generator (S : submodule α β) [S.is_principal] {x : β} :
  x ∈ S ↔ ∃ y : α, y • generator S = x :=
by rw [← submodule.mem_span_singleton, span_singleton_generator]

lemma mem_iff_generator_dvd (S : ideal α) [S.is_principal] {x : α} : x ∈ S ↔ generator S ∣ x :=
by rw [← ideal.mem_span_singleton, ideal.span, span_singleton_generator]

lemma eq_bot_iff_generator_eq_zero (S : submodule α β) [S.is_principal] :
  S = ⊥ ↔ generator S = 0 :=
iff.trans (by rw span_singleton_generator) (@span_singleton_eq_bot α _ _ _ _ _)
...
section
open euclidean_domain submodule
...
{ principal := λ S, by exactI
    ...
    else ⟨0, ext $ λ a, by rw [← @bot_coe α α _ _ ring.to_module, span_eq, mem_bot]; exact
...
⟨mt span_singleton_eq_top.1 hp.1, λ I hI, begin
  ...
  erw [span_singleton_le_span_singleton, mul_dvd_of_is_unit_right hb]
end⟩
...
/-- `factors a` gives a non-canonical factorization of `a` into irreducibles. -/
